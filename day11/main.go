...
func getCoordinates(size, serial int) (int, int, int) {
	...
	for i := 1; i <= gridSize; i++ {
		for j := 1; j <= gridSize; j++ {
			grid[i-1][j-1] = cellValue(j, i, serial)
		}
	}
...
func cellValue(x, y, serial int) int {
	rackID := x + 10
	powerLevel := rackID * y
	plusSerial := powerLevel + serial
